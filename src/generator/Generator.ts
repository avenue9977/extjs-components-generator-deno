...
export default class Generator {
	...
	public createComponentFiles() {
		...
		for (const file of filesToCreate) {
			...
			if (
				this.component.type === COMPONENT_TYPE.VIEW &&
				templateName.toLowerCase() !== COMPONENT_TYPE.VIEW &&
				!this.component.getViewConfig()
			...
			if (stats.isFile) {
				...
				) fileName = this.component.name + templateName;

				const writePath = `${pathToGenerate}/${
	...
	private getTemplateData(): TemplateData {
		const data: TemplateData = {
			...
			location: Generator.createTemplateLocationString(
		...
		if (this.component.type === COMPONENT_TYPE.STORE) {
			if (this.component.name.endsWith('s')) {
				...
				data.modelName = this.component.name.slice(
					0,
					this.component.name.length - 1,
				);
			} else {
